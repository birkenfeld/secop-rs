...
//! member called `pcache` of type `<Struct>ParamCache` (generated by the derive
    ...
    for p in params {
        ...
        par_write_arms.push(if p.readonly { quote! {
            ...
            #name => {
                ...
                let is_update = r_value != self.pcache.#name_id.0;
                let r_value_c = r_value.clone();
                self.#write_method(r_value)?;
                let tstamp = localtime();
                self.pcache.#name_id = (r_value_c, tstamp);
                if is_update {
        ...
        init_updates.push(quote! {
            ...
            if let Ok(value) = #type_static.to_json(self.pcache.#name_id.0.clone()) {
                res.push(Msg::Update { module: self.name().to_string(),
                                       param: #name.to_string(),
                                       data: json!([value, {"t": self.pcache.#name_id.1}]) });
            }
        });
    ...
    let generated_impl = input.gen_impl(quote! {
        ...
        gen impl secop_core::module::ModuleBase for @Self {
            ...
            fn poll_normal(&mut self, n: usize) {
                if (self.pcache.status.0).0 != StatusConst::Busy {
                    #( #poll_busy_params )*
                }
                #( #poll_other_params )*
            }
